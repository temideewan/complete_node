...
async function startServer() {
  try {
    ...
    // subscribe to events
    await consumeEvent('post.created', (args) => {
      args.redisClient = redisClient;
      return handlePostCreated(args);
    });
    await consumeEvent('post.deleted', (args) => {
      args.redisClient = redisClient;
      return handlePostDeleted(args);
    });
    app.listen(PORT, () => {
